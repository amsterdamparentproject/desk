...
// Shorthand helpers
const archived = (newsletter_last: string) => ({ status: 'archived', newsletter_last })
const staged   = (newsletter_last: string) => ({ status: 'edited',   newsletter_last }) // in next_newsletter, not archived
...
describe('computePublishDate', () => {
  ...
  it('computes last archived newsletter_last + 14 days', () => {
    const rows = [archived('2026-06-01')]
    expect(computePublishDate(rows, TODAY)).toBe('2026-06-15')
  })

  it('picks the max when multiple archived records have different newsletter_last dates', () => {
    const rows = [
      archived('2026-05-04'),
      archived('2026-06-01'), // most recent
      archived('2026-04-20'),
    ]
    expect(computePublishDate(rows, TODAY)).toBe('2026-06-15')
  })
  ...
  it('ignores newsletter_last from non-archived (next_newsletter) cards', () => {
    const rows = [
      archived('2026-06-01'), // last sent issue
  ...
  it('auto-advances by 14-day increments when the computed date is in the past', () => {
    ...
    const rows = [archived('2026-03-01')] // +14 = Mar 15, +14 = Mar 29, … keep going until > Jun 3
