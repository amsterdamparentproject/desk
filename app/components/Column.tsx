// components/Column.tsx
import { ChevronDown, ChevronRight } from 'lucide-react'
import { CompactCard } from './CompactCard'
import { AddCard } from './AddCard'
import { NewsletterEvent } from '../types/event'
import { InboxForm } from './InboxForm'

export function Column({ col, isOpen, onToggle, events, onDetails, onMove }) {
  return (
    <section className="flex flex-col border-b border-slate-200 md:border-r md:w-[350px] md:flex-shrink-0 md:border-b-0">
      {/* Header: Clickable on mobile, static-feeling on desktop */}
      <button 
        onClick={onToggle}
        // md:cursor-default disables the pointer feel on desktop since it's always open
        className={`w-full flex items-center justify-between p-4 transition-colors md:cursor-default ${
          isOpen ? 'bg-white' : 'bg-slate-50 md:bg-white'
        }`}
      >
        <div className="flex items-center gap-2">
          {/* Hide chevrons on desktop (md:hidden) */}
          <div className="md:hidden">
            {isOpen ? <ChevronDown size={16} className="text-slate-400" /> : <ChevronRight size={16} className="text-slate-400" />}
          </div>
          <h2 className="font-black text-xs uppercase tracking-tighter text-slate-700">
            {col.label}
          </h2>
        </div>
        
        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
          isOpen ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500 md:bg-slate-100'
        }`}>
          {events.length}
        </span>
      </button>

      {/* Content: Accordion on mobile, Fixed list on desktop */}
      <div className={`overflow-hidden transition-all duration-300 md:transition-none md:opacity-100 md:max-h-none ${
        isOpen ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'
      }`}>
        <div className="p-3 space-y-3 bg-slate-100 md:bg-transparent border-t border-slate-200 md:border-t-0 h-full overflow-y-auto">
          {col.id === 'incoming' && <InboxForm onAdd={onMove} />}
          {events.length === 0 ? (
            <div className="py-8 text-center text-[10px] font-bold text-slate-400 uppercase italic">
              No Events
            </div>
          ) : (
            events.map(event => (
              <CompactCard 
                key={event.id} 
                event={event} 
                onDetails={onDetails} 
                onMove={onMove} 
              />
            ))
          )}
        </div>
      </div>
    </section>
  )
}