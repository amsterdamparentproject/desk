...
export function DetailsForm({
  ...
  const descriptionStyle =
    'w-full text-sm leading-relaxed text-black border-none p-3 focus:ring-0 resize-none overflow-hidden bg-blue-50/80 rounded-lg'
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex justify-end">
      <div className="w-full max-w-4xl bg-white h-full overflow-y-auto shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="sticky top-0 bg-blue-600 border-b border-slate-100 p-4 flex items-center justify-between z-20">
          ...
          <button
            onClick={onClose}
            className="p-2 text-white hover:text-slate-200 rounded-full transition-colors"
          >
            <X size={20} />
          </button>
        </div>
          ...
          <section className="space-y-4">
            ...
            <Field label="Source URL">
              <div className="flex items-center gap-2">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-2 bg-slate-50 rounded-lg hover:bg-blue-50 text-blue-600 transition-colors"
                >
            ...
            <Field label="Newsletter blurb">
              <textarea
                ...
                onChange={(e) =>
                  handleChange('newsletterDescription', e.target.value)
                }
            ...
            <div className="grid grid-cols-3 gap-8 pb-2">
              <Field label="Start Date">
                <input
                  ...
                  className="w-full text-sm font-bold text-slate-700 border-none p-0 focus:ring-0"
                />
              </Field>
            ...
            <div className="grid grid-cols-3 gap-4 items-end pb-2">
              <Field label="Repeat frequency">
                <select
                  ...
                  onChange={(e) =>
                    handleChange('repeatFrequency', e.target.value)
                  }
              ...
              <Field label="Repeat description">
                <input
                  placeholder="Every first Sunday"
            ...
            <div>
              <CheckboxField
                ...
                onChange={(value: boolean) =>
                  handleChange('add_to_calendar', value)
                }
              />
            </div>
          </section>
            ...
            <div className="grid grid-cols-2 gap-8">
              <Field label="Area">
                <select
                  ...
                  className="w-full text-sm font-bold text-slate-700 border-none p-0 focus:ring-0 bg-transparent cursor-pointer"
                >
        ...
        <div className="sticky bottom-0 bg-white border-t border-slate-100 p-6 z-20 grid grid-cols-2 gap-8">
          <button
            ...
            className="w-full bg-slate-900 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-blue-600 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-xs"
          >
            <Save size={18} strokeWidth={3} /> Save
          </button>
          <button
            onClick={() => onSave(formData)}
            className="w-full bg-green-600 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-blue-600 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-xs"
          >
    ...
    <div className="flex flex-col gap-1.5 group">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5 text-slate-400 group-focus-within:text-blue-500">
          {icon}{' '}
          <label className="text-[9px] text-green-600 font-black uppercase tracking-widest">
            {label}
          </label>
        </div>
        {extra}
...
function CheckboxField({
  ...
  return (
    <button
      ...
      className={`flex items-center justify-between w-full p-3 rounded-xl border transition-all text-left ${checked ? activeColor : inactiveColor}`}
    >
      <div className="flex items-center gap-3 overflow-hidden">
        <div
          className={`p-2 rounded-lg ${checked ? 'bg-white/50' : 'bg-slate-50'}`}
        >
          {icon}
        </div>
